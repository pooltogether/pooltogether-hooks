...
export const useLPTokenUsdValue = (prizePool: V3PrizePool) => {
  ...
  } = useCoingeckoTokenPrices(prizePool.chainId, [token1Address, token2Address])

  const {
    data: lPTokenBalances,
    isFetched: tokenBalancesIsFetched
  } = useTokenBalances(prizePool.chainId, lpPrizePoolMetadata.tokens.underlyingToken.address, [
    token1Address,
    token2Address,
    lpTokenAddress
  ])
...
export const getLPPrizePoolMetadata = (prizePool: V3PrizePool) => {
  return V3.LP_PRIZE_POOL_METADATA[prizePool.chainId]?.find(
